...
async function start() {
  ...
  // 1. Get the slider element HERE, inside the start function.
  const sensitivitySlider = document.getElementById('sensitivitySlider');

  if (sensitivitySlider && wasm.set_sensitivity) {
    // 2. Define constants here so they are visible to all the code below.
    const minDamp = 5.0;
    const maxDamp = 40.0;

    // 3. Add the event listener.
    sensitivitySlider.addEventListener('input', () => {
      const sliderValue = parseFloat(sensitivitySlider.value);
      const sensitivityValue = maxDamp - sliderValue + minDamp;
      wasm.set_sensitivity(sensitivityValue);
    }, false);

    // 4. Set the initial value correctly.
    const initialSliderValue = parseFloat(sensitivitySlider.value);
    const initialSensitivity = maxDamp - initialSliderValue + minDamp;
    wasm.set_sensitivity(initialSensitivity);
  }
